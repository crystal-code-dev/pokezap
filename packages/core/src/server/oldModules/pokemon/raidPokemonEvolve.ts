...
import { RaidPokemonBaseData } from '../../../../../prisma-provider/src/types'
...
export const raidPokemonEvolve = async (
  poke: RaidPokemonBaseData,
  maximumBaseExperience: number
): Promise<RaidPokemonBaseData> => {
  ...
  const evolvedPoke = poke.isShiny
    ? await prisma.raidPokemon.update({
        where: {
          id: poke.id,
        },
        data: {
